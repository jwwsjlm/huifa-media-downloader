...
构建及打包烟雾检查，然后发布 `HuifaVideoDownloader.exe` 和
`SHA256SUMS.txt` 到 GitHub Release。应用默认通过 GitHub REST API
...
当前主发行版是单文件 `releases/HuifaVideoDownloader.exe`，直接双击即可运行，也可以从任意
工作目录启动。PySide6/Qt 是本程序界面运行时，不能从程序内部删除；内置 yt-dlp 模块和 FFmpeg
...
开发者可使用 `scripts/build_release.ps1` 重建发行版；脚本会先运行编译检查和单元测试，
再生成唯一交付文件 `releases/HuifaVideoDownloader.exe`，不再创建发行 ZIP 或附带目录。
...
最后还会执行交付目录校验：
`releases/` 顶层文件必须只有非空的 `HuifaVideoDownloader.exe`，不会生成、复制或保留发行 ZIP；
`releases/data/` 等本地运行目录不参与交付，也不会被构建脚本删除。发布到 GitHub 时只选择该 EXE。

单 EXE 不使用 Velopack，因为 Velopack 的更新单位是完整的 onedir 应用目录。主发行版使用
...
只提示一次真实安装结果，避免把“已下载”误认为“已安装”。仓库仍保留可选的 Velopack
安装版/目录便携版构建基础，二者共享检查、下载、更新内容、确认界面和安装结果回执，但使用
各自适合的替换机制。
