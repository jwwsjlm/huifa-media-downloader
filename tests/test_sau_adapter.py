...
class SauCapabilityTests(unittest.TestCase):
    @staticmethod
    def payload(platform: str, settings: dict | None = None) -> dict:
        platform_settings = {"account": "work", **(settings or {})}
        if platform == "bilibili":
            platform_settings.setdefault("tid", "17")
        return {
            "media": {
                "video_path": "C:/media/video.mp4",
                "thumbnail_path": "C:/media/default.jpg",
            },
            "metadata": {
                "title": "演示标题",
                "description": "演示简介",
    ...
    def test_official_platform_list_and_display_names_are_centralized(self) -> None:
        ...
        self.assertIs(
            get_sau_platform_capability(" YouTube "),
            SAU_PLATFORM_CAPABILITIES["youtube"],
        )
    ...
    def test_capability_sets_match_the_vendored_parser(self) -> None:
        schedules = {
            name
            for name, item in SAU_PLATFORM_CAPABILITIES.items()
            if item.supports_schedule
        }
        collections = {
            name
            for name, item in SAU_PLATFORM_CAPABILITIES.items()
            if item.supports_collection
        }
        dual_covers = {
            name
            for name, item in SAU_PLATFORM_CAPABILITIES.items()
            if item.supports_dual_thumbnail
        }
        visibility = {
            name
            for name, item in SAU_PLATFORM_CAPABILITIES.items()
            if item.supports_visibility
        }
        playlists = {
            name
            for name, item in SAU_PLATFORM_CAPABILITIES.items()
            if item.supports_playlist
        }
        required_tid = {
            name
            for name, item in SAU_PLATFORM_CAPABILITIES.items()
            if item.tid_required
        }
        self.assertEqual(
            schedules, {"douyin", "kuaishou", "xiaohongshu", "bilibili", "tencent"}
        )
        self.assertEqual(
            collections,
            {"douyin", "kuaishou", "tencent", "baijiahao", "alipay", "weibo"},
        )
    ...
    def test_all_platforms_build_structured_upload_requests(self) -> None:
        for platform in SAU_SUPPORTED_PLATFORMS:
            with self.subTest(platform=platform):
                request = build_upload_request(platform, self.payload(platform))
    ...
    def test_all_structured_requests_are_accepted_by_the_real_vendored_dispatcher(
        self,
    ) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for platform in SAU_SUPPORTED_PLATFORMS:
                ...
                request = build_upload_request(platform, payload)
                ...
                with self.subTest(platform=platform), patch.object(
                    ...
                    result = asyncio.run(
                        module.publish_video_payload(platform, request)
                    )
    ...
    def test_account_actions_call_runtime_directly_for_every_platform(self) -> None:
        for platform in SAU_SUPPORTED_PLATFORMS:
            with self.subTest(platform=platform, action="login"), patch(
                "app.adapters.sau_adapter.probe_sau_compatibility",
                return_value=SauCoreCompatibility(True, platform, "login"),
            ), patch(
                "app.adapters.sau_adapter.account_login",
                return_value={"success": True, "message": "logged in"},
            ) as login, patch(
                "app.adapters.sau_adapter.account_check"
            ) as check:
                ok, message = account_action(platform, "login", "work")
                self.assertTrue(ok)
                self.assertEqual(message, "logged in")
                login.assert_called_once_with(
                    platform, "work", headed=True, cancel_event=None
                )
                check.assert_not_called()

            with self.subTest(platform=platform, action="check"), patch(
                "app.adapters.sau_adapter.probe_sau_compatibility",
                return_value=SauCoreCompatibility(True, platform, "check"),
            ), patch("app.adapters.sau_adapter.account_login") as login, patch(
                "app.adapters.sau_adapter.account_check", return_value=True
            ) as check:
                ok, message = account_action(platform, "check", "work")
                self.assertTrue(ok)
    ...
    def test_action_probe_uses_core_status(self) -> None:
        with patch(
            "app.adapters.sau_adapter.core_status", return_value=(True, "ready")
        ):
            action = probe_sau_compatibility("douyin", "upload-video")
        self.assertTrue(action.compatible)

        with patch(
            "app.adapters.sau_adapter.core_status",
            return_value=(False, "missing playwright"),
        ):
    ...
    def test_cancel_event_and_sensitive_output_are_propagated_safely(self) -> None:
        compatibility = SauCoreCompatibility(True, "douyin", "login")
        ...
        with patch(
            ...
            ok, message = account_action("douyin", "login", "work", cancel_event=cancel)
        self.assertFalse(ok)
        self.assertIn("取消", message)
        login.assert_called_once_with(
            "douyin", "work", headed=True, cancel_event=cancel
        )
        ...
        with patch(
            ...
            ok, message = account_action("douyin", "login", "work")
        self.assertFalse(ok)
    ...
    def test_publish_calls_runtime_without_subprocess(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            compatibility = SauCoreCompatibility(True, "douyin", "upload-video")
            with patch(
                "app.adapters.sau_adapter.probe_sau_compatibility",
                return_value=compatibility,
            ), patch(
                "app.adapters.sau_adapter.publish_video",
                return_value="douyin published",
            ) as publish_video:
                ok, message = publish("douyin", payload)
        self.assertTrue(ok)
        self.assertEqual(message, "douyin published")
        publish_video.assert_called_once()
        self.assertEqual(publish_video.call_args.args[0], "douyin")
        self.assertEqual(publish_video.call_args.args[1]["account_name"], "work")
        self.assertEqual(publish_video.call_args.kwargs, {"cancel_event": None})
    ...
    def test_dual_cover_schedule_visibility_playlist_and_tid_arguments(self) -> None:
        ...
        request = build_upload_request("douyin", payload)
        ...
        payload = self.payload(
            "youtube", {"collection": "Playlist A", "visibility": "unlisted"}
        )
        request = build_upload_request("youtube", payload)
        self.assertEqual(request["playlist"], "Playlist A")
        self.assertEqual(request["visibility"], "unlisted")

        payload = self.payload("bilibili", {"tid": "171"})
        request = build_upload_request("bilibili", payload)
    ...
    def test_invalid_schedule_tid_and_visibility_return_clear_errors(self) -> None:
        for value in ("2026/08/24 09:30", "2026-02-30 09:30", "2026-08-24 25:00"):
            payload = self.payload("douyin", {"schedule": value})
            with self.subTest(schedule=value), self.assertRaisesRegex(
                ValueError, "定时发布时间"
            ):
                build_upload_request("douyin", payload)

        missing_tid = self.payload("bilibili", {"tid": ""})
        with self.assertRaisesRegex(ValueError, "tid"):
            build_upload_request("bilibili", missing_tid)

        payload = self.payload("youtube", {"visibility": "friends-only"})
        with self.assertRaisesRegex(ValueError, "public"):
            build_upload_request("youtube", payload)
...
class SocialAutoUploadRuntimeTests(unittest.TestCase):
    ...
    def test_real_vendored_core_imports_with_official_playwright(self) -> None:
        ...
        self.assertTrue(
            all(hasattr(module, name) for name in runtime._LOGIN_FUNCTIONS.values())
        )
        self.assertTrue(
            all(hasattr(module, name) for name in runtime._CHECK_FUNCTIONS.values())
        )
