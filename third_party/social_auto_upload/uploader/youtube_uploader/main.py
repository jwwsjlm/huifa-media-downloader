...
try:
    # 国内直连 youtube.com 会超时，且 Playwright 启动的 Chromium 不自动继承系统代理。
...
def _launch_kwargs(headless: bool, *, proxy=None) -> dict:
    """Use Huifa's app-local Chromium and never a user Chrome installation."""
    kwargs = {"headless": headless, "executable_path": LOCAL_CHROME_PATH}
    if proxy:
        kwargs["proxy"] = proxy
    return kwargs
...
async def _is_youtube_studio_signed_in(page, context) -> bool:
    """Detect a usable Studio session without relying on one URL shape."""
    if page.is_closed():
        return False
    ...
    if "studio.youtube.com" not in lowered_url:
        return False
    if "/channel/" in lowered_url:
        return True
    try:
        if await page.locator("ytcp-app, ytcp-entity-page").count() > 0:
            return True
    except Exception:
        pass
    try:
...
async def cookie_auth(account_file) -> bool:
    ...
    async with async_playwright() as playwright:
        browser = await playwright.chromium.launch(**_launch_kwargs(True))
        try:
            context = await browser.new_context(storage_state=account_file)
            context = await set_init_script(context)
            page = await context.new_page()
            await page.goto(STUDIO_URL, wait_until="commit", timeout=15000)
            await page.wait_for_timeout(3000)
            return await _is_youtube_studio_signed_in(page, context)
        except Exception:
            return False
        finally:
            await browser.close()
...
async def youtube_cookie_gen(
    account_file,
    headless: bool = False,
    *,
    max_wait_seconds: int = 600,
):
    """Open a blank app-owned browser and persist cookies from visited sites."""
    async with async_playwright() as playwright:
        # Cookie capture is interactive. Start with a neutral blank tab so the
        # user decides which site to visit instead of automatically contacting
        # YouTube every time the browser opens.
        browser = await playwright.chromium.launch(**_launch_kwargs(False))
        account_path = Path(account_file)
        account_path.parent.mkdir(parents=True, exist_ok=True)
        context_options = {"storage_state": str(account_path)} if account_path.is_file() else {}
        try:
            context = await browser.new_context(**context_options)
        except Exception as exc:
            if not context_options:
                raise
            # A truncated state file must not prevent the visible login window
            # from opening.  Keep the original file for diagnostics and start
            # a clean in-memory session that will replace it after login.
            youtube_logger.warning(_msg("⚠️", f"旧登录态无法读取，将打开全新登录会话: {exc}"))
            context = await browser.new_context()
        context = await set_init_script(context)
        page = await context.new_page()
        ok = False
        status = "timeout"
        message = "浏览器会话超时，未捕获到 Cookie"
        current_url = "about:blank"
        saved_cookie_count = 0
        try:
            youtube_logger.info(_msg("🔐", "浏览器已打开空白页，请在地址栏输入需要登录的网站；Cookie 会自动保存"))
            for _ in range(max(1, int(max_wait_seconds))):
                ...
                if active_page is not None:
                    current_url = str(active_page.url or current_url)
                try:
                    cookies = await context.cookies()
                except Exception:
                    cookies = []
            ...
            ok = saved_cookie_count > 0
            if ok:
                status = "cookies_saved"
                message = f"已保存 {saved_cookie_count} 条 Cookie"
                youtube_logger.success(_msg("✅", f"浏览器 Cookie 已保存: {account_path}"))
            elif status == "closed":
                message = "浏览器窗口已关闭，但没有捕获到 Cookie"
            else:
                youtube_logger.error(_msg("😵", "浏览器会话超时，未捕获到 Cookie"))
        finally:
            if browser.is_connected():
                pages = [candidate for candidate in context.pages if not candidate.is_closed()]
                if pages:
                    current_url = str(pages[-1].url or current_url)
            try:
                await browser.close()
            except Exception:
                pass
        return _build_login_result(ok, status, message, account_file, current_url)
